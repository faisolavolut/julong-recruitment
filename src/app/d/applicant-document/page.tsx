...
function Page() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      local.can_add = getAccess("create-applicant-document", roles);
      local.can_edit = getAccess("edit-applicant-document", roles);
      local.render();
    };
    run();
  }, []);
    ...
    <div className="flex p-4 flex-col flex-grow bg-white rounded-lg border border-gray-300 shadow-md shadow-gray-300">
      <div className="flex flex-col py-4 pb-0 pt-0">
        <h2 className="text-xl font-semibold text-gray-900 ">
          <span className="">Applicant Document</span>
        </h2>
      </div>
              ...
              renderCell: ({ row }: any) => {
                return (
                  <div className="flex items-center gap-x-0.5 whitespace-nowrap">
                    {local.can_edit ? (
                      <ButtonLink href={`/d/applicant-document/${row.id}/edit`}>
                      ...
                      <ButtonLink
                        className="bg-primary"
                        href={`/d/applicant-document/${row.id}/view`}
                      >
                        <div className="flex items-center gap-x-2">
